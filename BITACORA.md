...
## 2026-08-16 (cont. 2) — RF-005/006/007/009 implementados, RF-013/014 documentados
...
- Con esto, v0.1 del roadmap tiene sus 6 RFs con código real: RF-001/002/005/006/007 en progreso (bloqueados en un `SyncProvider`/Sync Engine real), RF-009 implementado. Al terminar RF-009 quedó claro que faltaba una pieza no listada en RF-001..012 original — documentados **RF-013** (`DropboxProvider`, implementación concreta de `SyncProvider`) y **RF-014** (Sync Engine, el orquestador del ciclo — v0.1 sin resolución de conflictos, esa es v0.2) en rama `docs/rf-013-014-sync-engine`. Solo documentación (RO-003), sin implementar. `ROADMAP.md` (es/en) actualizado: v0.1 ahora lista 8 RFs, con RF-009 tildado.
- Merge de todo lo pendiente (PRs #4→#5→#6→#7 apilados, retargeteados a `develop` uno por uno tras mergear el anterior, y #8 independiente) — con `develop` como único punto de verdad de acá en adelante.
